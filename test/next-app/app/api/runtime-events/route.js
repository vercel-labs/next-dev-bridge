...
function getStore() {
  if (!globalThis.__NEXT_HMR_OBSERVER_RUNTIME_EVENTS__) {
    globalThis.__NEXT_HMR_OBSERVER_RUNTIME_EVENTS__ = {
  ...
  return globalThis.__NEXT_HMR_OBSERVER_RUNTIME_EVENTS__
}
...
function normalizeFrameFile(file) {
  ...
  try {
    ...
    if (frameUrl.pathname.startsWith('/_next/static/')) {
      ...
      return (
        pathToFileURL(path.join(getNextDevDistDir(), distRelativePath))
          .href + frameUrl.search
      )
    }
  } catch {}
...
function getNextDevDistDir() {
  ...
  // Next 16 dev output lives under .next/dev. Older dev output used .next.
