...
Observe a running Next.js dev server and receive readable build/runtime state
events.

`nexto` is focused on the dev overlay use case: know when a Next app has build
...
Then watch it with `nexto`:

```sh
nexto http://localhost:3000
```
...
```sh
nexto 3000
nexto watch http://localhost:3000 --verbose
nexto http://localhost:3000 --no-reconnect
```
...
nexto asks Next.js to decode captured runtime errors and uses the mapped frames
when available.
...
messages yourself and want nexto to normalize them.
...
apply edits, show the preview in an iframe, and render nexto events.
